...
gh workflow list --repo NachoColl/mateocoll-website
```
...
gh run list --branch $CURRENT_BRANCH --limit 50 --repo NachoColl/mateocoll-website \
  --json databaseId,status,conclusion,name,workflowName,createdAt \
  --jq "[.[] | select(.createdAt >= \"$MONITORING_START\")]"

# Monitor each workflow (extract IDs from above)
for RUN_ID in {run_ids}; do
    gh run watch $RUN_ID --repo NachoColl/mateocoll-website
done

# Check for cascading workflows after 30 seconds
sleep 30
gh run list --branch $CURRENT_BRANCH --limit 20 --repo NachoColl/mateocoll-website \
  --json databaseId,status,conclusion,name,workflowName,createdAt
```
...
```bash
# Watch in real-time
gh run watch --repo NachoColl/mateocoll-website

# View specific run logs
gh run view <run-id> --log --repo NachoColl/mateocoll-website
```

### Re-run Failed Workflow

```bash
gh run rerun <run-id> --failed --repo NachoColl/mateocoll-website
```
